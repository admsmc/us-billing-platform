...
class WorkerInternalAuthFilter(
    private val props: WorkerInternalAuthProperties,
    private val objectMapper: ObjectMapper,
) : OncePerRequestFilter() {

    private val logger = LoggerFactory.getLogger(WorkerInternalAuthFilter::class.java)

    override fun shouldNotFilter(request: HttpServletRequest): Boolean {
        val path = request.requestURI ?: ""
        // Only guard internal operational endpoints.
    ...
    override fun doFilterInternal(request: HttpServletRequest, response: HttpServletResponse, filterChain: FilterChain) {
        ...
        if (bearer == null || jwtKeyring.isEmpty()) {
            SecurityAuditLogger.internalAuthFailed(
                component = "payroll-worker-service",
