...
class PayRunRepository(
    ...
    fun createOrGetPayRun(
        ...
        try {
            ...
            // If idempotency key hit, return the existing run.
            if (requestedIdempotencyKey != null) {
                return findByIdempotencyKey(employerId, requestedIdempotencyKey)
                    ?: throw e
            }
            // Otherwise, assume duplicate pay_run_id.
        }

        return findPayRun(employerId, payRunId)
    ...
    fun tryAcquireLease(
        ...
        val updated = jdbcTemplate.update(
            ...
            WHERE employer_id = ?
              AND pay_run_id = ?
              AND (lease_expires_at_epoch_ms IS NULL OR lease_expires_at_epoch_ms < ?)
              AND status IN (?, ?)
            """.trimIndent(),
            leaseOwner,
            leaseUntilEpoch,
