...
class RateContextService(
    ...
    private fun toRegulatoryCharge(entity: com.example.usbilling.tax.domain.TariffRegulatoryChargeEntity): RegulatoryCharge = RegulatoryCharge(
    ...
    private fun parseRegulatoryChargeType(value: String): RegulatoryChargeType = try {
        RegulatoryChargeType.valueOf(value.uppercase())
    } catch (e: IllegalArgumentException) {
        RegulatoryChargeType.FIXED
    }

    private fun inferUnit(serviceType: String): String = when (serviceType.uppercase()) {
        "ELECTRIC" -> "kWh"
        "GAS" -> "therms"
        "WATER" -> "CCF"
        "WASTEWATER" -> "CCF"
