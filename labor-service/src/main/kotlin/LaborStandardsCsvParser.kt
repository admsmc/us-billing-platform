...
/**
 * Parser for labor standards CSVs.
 *
 * There are two CSV shapes:
 *
 *  1. Statewide baseline: labor-standards-YYYY.csv
 *     Columns:
 ...
 *       - state_statute_url
 *
 *  2. Local/regional overrides: labor-standards-YYYY-local.csv
 *     Columns:
 *       - state_code
 *       - locality_code (e.g. NYC, SEA, PORTLAND_METRO)
...
object LaborStandardsCsvParser {
    ...
    /**
     * Parse locality-specific labor standards from labor-standards-YYYY-local.csv.
     */
    fun parseLocal(reader: Reader): List<StateLaborStandard> {
        val parser = csvFormat.parse(reader)
        return parser.map { toLocalStandard(it) }
    }
    ...
    private fun toLocalStandard(rec: CSVRecord): StateLaborStandard {
        val stateCode = rec.getRequired("state_code").trim().uppercase()
        val localityCode = rec.getRequired("locality_code").trim().uppercase()
        val localityKind = rec.getRequired("locality_kind").trim().uppercase()
